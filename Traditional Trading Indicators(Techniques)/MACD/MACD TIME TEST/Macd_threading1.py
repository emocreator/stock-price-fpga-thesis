...
# Example of multithreading the MACD calculation by dividing the data
segments = [Close_prices[i:i+500] for i in range(0, len(Close_prices), 500)]  # Example segmentation
results = []
with ThreadPoolExecutor(max_workers=4) as executor:
    futures = [executor.submit(calculate_macd, segment, 12, 26, 9) for segment in segments]
    for future in futures:
        results.append(future.result())
macd = [item for sublist in results for item in sublist[0]]
macd_signal = [item for sublist in results for item in sublist[1]]
macd_hist = [item for sublist in results for item in sublist[2]]
